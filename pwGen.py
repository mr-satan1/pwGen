...
def randomString(length, outputQueue):
    stringy = ''.join(random.choice(string.ascii_uppercase + string.digits + string.ascii_lowercase + string.punctuation) for _ in range(15))
    outputQueue.put(stringy)

# Set up a list of processes that we want to run
processes = [mp.Process(target=randomString, args=(5, outputQueue)) for x in range(4)]
